...
const HomeView: React.FC<HomeViewProps> = ({ myTracks, myGroupHikes, onPublishTrack, onDeleteGroupHike }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 pb-24 overflow-y-auto">
      {/* Header Profile */}
      <div className="bg-white px-4 sm:px-6 pt-6 sm:pt-8 pb-8 sm:pb-10 rounded-b-3xl shadow-sm relative">
         <div className="absolute -top-8 right-0 p-4 opacity-10">
            <Map size={200} />
         </div>

         {/* Profile Row - Improved responsive layout */}
         <div className="flex justify-between items-start gap-3 mb-4 sm:mb-6 relative z-10">
            <div className="flex items-center gap-3 min-w-0 flex-1">
               <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full border-4 border-hike-green p-1 flex-shrink-0">
                  <img src={profileAvatarUrl} className="w-full h-full rounded-full object-cover" alt="Profile" />
               </div>
               <div className="min-w-0">
                  <h1 className="text-lg sm:text-2xl font-bold text-gray-900 truncate mb-1">{profileUsername}</h1>
            ...
            <div className="flex gap-2 flex-shrink-0">
               <button className="p-2 bg-gray-100 rounded-full text-gray-600 hover:bg-gray-200 transition"><QrCode size={18}/></button>
               <button
         ...
         <div className="text-center relative z-10 mt-3">
            <div className="text-xs sm:text-sm text-gray-500 uppercase tracking-wide mb-1">Total Distance</div>
            <div className="text-4xl sm:text-5xl font-black text-hike-dark font-mono tracking-tighter">
               {stats.totalDistanceKm.toFixed(1)}<span className="text-sm sm:text-lg text-gray-400 ml-1 font-sans font-normal">km</span>
            </div>
         </div>
      </div>

      {/* Body sections */}
      <div className="flex-1 px-4 pt-4 pb-8 space-y-6 mt-4">
        {/* Stats Grid */}
        <div className="grid grid-cols-2 gap-3">
         <div className="bg-white p-4 rounded-3xl shadow-md border border-gray-100 flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <div className="bg-white rounded-3xl shadow-md border border-gray-100 p-5">
         ...
         <div className="space-y-3">
             {myTracks.length === 0 && (
                 <div className="text-center text-gray-400 py-6 text-sm bg-white rounded-xl border border-dashed">No recorded tracks yet.</div>
             )}

             {myTracks.map(track => (
                <div key={track.id} className="bg-white p-4 rounded-2xl shadow-md border border-gray-100 flex items-center gap-3 animate-fade-in">
             ...
             {/* Static Legacy Item */}
             <div className="bg-white p-4 rounded-2xl shadow-md border border-gray-100 flex items-center gap-3 opacity-70">
        ...
        <div className="bg-white rounded-3xl shadow-md border border-gray-100 p-5">
         ...
         <div className="space-y-3">
             {myGroupHikes.length === 0 ? (
                 <div className="text-center text-gray-400 py-6 text-sm bg-white rounded-xl border border-dashed">
                     No upcoming group hikes. Organize one in "Start Hiking"!
                 </div>
             ) : (
                 myGroupHikes.map(hike => (
                     <div key={hike.id} className="bg-orange-50 p-4 rounded-2xl shadow-md border border-orange-100 flex items-center gap-3 animate-fade-in">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-gray-600 flex gap-2 mt-1 flex-wrap">
                                <span className="flex items-center gap-1 whitespace-nowrap"><Clock size={10}/> {hike.date}</span>
                                <span className="flex items-center gap-1 whitespace-nowrap">Member: {hike.currentMembers}/{hike.maxMembers}</span>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0">
                            <span className="text-[10px] bg-orange-200 text-orange-800 px-2 py-1 rounded font-bold whitespace-nowrap">
                                OWNER
                            </span>
                            <button
                              onClick={() => onDeleteGroupHike?.(hike.id)}
